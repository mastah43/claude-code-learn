...
class GraphEnhancedSearchTool(Tool):
    """Enhanced search tool that combines vector search with knowledge graph
    traversal"""

    def __init__(
        self, vector_store: VectorStore, graph_store: Optional[GraphStore] = None
    ):
    ...
    def get_tool_definition(self) -> Dict[str, Any]:
        ...
        if self.graph_enhanced:
            description += (
                ". Includes related content through knowledge graph connections."
            )
            ...
            "input_schema": {
                ...
                "properties": {
                    ...
                    "include_related": {
                        "type": "boolean",
                        "description": (
                            "Whether to include semantically related "
                            "chunks via knowledge graph (default: true)"
                        ),
                    },
                    "max_related_chunks": {
                        "type": "integer",
                        "description": (
                            "Maximum number of related chunks to include "
                            "(default: 3)"
                        ),
                    },
                },
    ...
    def execute(
        ...
        if not self.graph_enhanced or not include_related or primary_results.is_empty():
            return self._format_traditional_results(
                primary_results, course_name, lesson_number
            )
    ...
    def _perform_graph_enhanced_search(
        self, primary_results: SearchResults, max_related_chunks: int
    ) -> Dict[str, Any]:
        ...
        for chunk_id in primary_chunk_ids:
            ...
            for entity in entities_in_chunk:
                ...
                related_chunks = self.graph_store.get_chunks_for_entities(
                    related_entity_ids
                )
    ...
    def _fetch_chunk_content(self, chunk_ids: List[str]) -> List[Dict[str, Any]]:
        ...
        for chunk_id in chunk_ids:
            try:
                ...
                course_title_encoded.replace("_", " ")

                # Search for this specific chunk
                results = self.vector_store.course_content.get(ids=[chunk_id])
    ...
    def _format_traditional_results(
        self,
    ...
    def _format_enhanced_results(
        self, enhanced_results: Dict[str, Any], primary_results: SearchResults
    ) -> str:
        ...
        if num_related > 0:
            summary = (
                f"\n\n[GraphRAG Enhanced] Found {num_related} related chunks "
                f"through knowledge graph connections."
            )
