...
class GraphStore:
    ...
    def add_relationship(self, relationship: Relationship) -> None:
        ...
        if (
            relationship.source_entity_id not in self.entities
            or relationship.target_entity_id not in self.entities
        ):
            raise ValueError("Both entities must exist before adding relationship")
    ...
    def get_related_entities(
        ...
        while queue:
            ...
            neighbors = set(self.graph.successors(current_id)) | set(
                self.graph.predecessors(current_id)
            )
            ...
            for neighbor_id in neighbors:
                if neighbor_id not in visited:
                    ...
                    if relation_types:
                        # Check if any edge between current and neighbor has
                        ...
                        if edges and any(
                            edge_data.get("relation_type") in relation_types
    ...
    def find_shortest_path(
        ...
        try:
            return nx.shortest_path(
                self.graph.to_undirected(), source_entity_id, target_entity_id
            )
