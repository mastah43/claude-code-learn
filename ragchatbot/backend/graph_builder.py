...
class GraphBuilder:
    ...
    def build_graph_from_chunks(self, chunks: List[CourseChunk]) -> GraphStore:
        ...
        for i, chunk in enumerate(chunks):
            ...
            chunk_relationships = self.entity_extractor.extract_relationships(
        ...
        merged_relationships = self.entity_extractor.merge_relationships(
            all_relationships
        )
    ...
    def update_graph_with_new_chunks(
        self, new_chunks: List[CourseChunk], existing_graph: GraphStore
    ) -> GraphStore:
        ...
        new_merged_relationships = self.entity_extractor.merge_relationships(
            all_relationships
        )
    ...
    def get_graph_summary(self) -> Dict:
        ...
        try:
            ...
            for entity_id, score in top_entities:
                ...
                if entity:
                    top_entity_names.append(
                        f"{entity.name} ({entity.entity_type}, {score:.3f})"
                    )
        except Exception:
            top_entity_names = []
    ...
    def find_chunks_by_entity_name(
