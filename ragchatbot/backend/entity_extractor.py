...
from models import Entity, Relationship, EntityType, RelationType, CourseChunk
...
class EntityExtractor:
    ...
    def __init__(self):
        ...
        self.tech_keywords = {
            ...
            'react', 'vue', 'angular', 'django', 'flask', 'fastapi', 'node.js', 'express',
            'postgresql', 'mysql', 'mongodb', 'redis', 'elasticsearch', 'docker', 'kubernetes',
            'aws', 'azure', 'gcp', 'tensorflow', 'pytorch', 'scikit-learn', 'pandas', 'numpy',
            'git', 'github', 'gitlab', 'jenkins', 'terraform', 'ansible', 'api', 'rest', 'graphql',
            'ai', 'ml', 'machine learning', 'artificial intelligence', 'neural network', 'llm',
            'rag', 'retrieval augmented generation', 'vector database', 'embedding', 'transformer'
        }
        
        # Tool-related keywords
        self.tool_keywords = {
            'vscode', 'pycharm', 'intellij', 'eclipse', 'vim', 'emacs', 'sublime text',
            'postman', 'curl', 'wget', 'jupyter', 'colab', 'notebook', 'terminal', 'cli',
            'bash', 'powershell', 'ssh', 'ftp', 'sftp', 'rsync', 'wget', 'pip', 'npm', 'yarn',
            'conda', 'virtualenv', 'pipenv', 'poetry', 'make', 'cmake', 'gradle', 'maven'
        }
        
        # Method/concept keywords
        self.method_keywords = {
            'algorithm', 'data structure', 'sorting', 'searching', 'recursion', 'iteration',
            'oop', 'object oriented', 'functional programming', 'design pattern', 'mvc',
            'microservices', 'monolith', 'crud', 'authentication', 'authorization', 'encryption',
            'hashing', 'caching', 'optimization', 'refactoring', 'testing', 'debugging',
            'deployment', 'ci/cd', 'devops', 'agile', 'scrum', 'kanban', 'tdd', 'bdd'
        }
        
        # Organization keywords
        self.org_keywords = {
            'google', 'microsoft', 'amazon', 'meta', 'facebook', 'apple', 'netflix', 'uber',
            'airbnb', 'twitter', 'linkedin', 'github', 'gitlab', 'stackoverflow', 'reddit',
            'openai', 'anthropic', 'deepmind', 'nasa', 'mit', 'stanford', 'berkeley', 'cmu'
        }
    ...
    def _extract_organization_entities(self, content: str, chunk_id: str) -> List[Entity]:
    ...
    def _extract_course_lesson_entities(self, chunk: CourseChunk, chunk_id: str) -> List[Entity]:
        """Extract course and lesson entities"""
        entities = []
        
        # Course entity
        course_entity_id = self._generate_entity_id(chunk.course_title, EntityType.COURSE)
        entities.append(Entity(
            id=course_entity_id,
    ...
    def _extract_code_entities(self, content: str, chunk_id: str) -> List[Entity]:
        ...
        for match in set(caps_matches):
            if len(match) > 2 and match not in {'API', 'URL', 'HTTP', 'JSON', 'XML', 'CSS', 'SQL'}:
                entity_id = self._generate_entity_id(match, EntityType.CONCEPT)
                entities.append(Entity(
                    id=entity_id,
                    name=match,
    ...
    def extract_relationships(self, entities: List[Entity], chunk: CourseChunk) -> List[Relationship]:
        ...
        if EntityType.COURSE in entities_by_type and EntityType.LESSON in entities_by_type:
        ...
        for course_lesson_type in [EntityType.COURSE, EntityType.LESSON]:
            if course_lesson_type in entities_by_type:
                for content_type in [EntityType.TECHNOLOGY, EntityType.TOOL, EntityType.METHOD]:
                    if content_type in entities_by_type:
                        for course_lesson_entity in entities_by_type[course_lesson_type]:
        ...
        if EntityType.TECHNOLOGY in entities_by_type and EntityType.TOOL in entities_by_type:
    ...
    def merge_relationships(self, relationship_list: List[List[Relationship]]) -> List[Relationship]:
