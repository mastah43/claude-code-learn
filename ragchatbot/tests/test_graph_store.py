...
class TestGraphStore:
    ...
    def test_add_multiple_entities(self, sample_entities):
        ...
        assert all(self.graph_store.graph.has_node(entity.id) for entity in sample_entities)
    ...
    def test_add_relationship(self, sample_entities, sample_relationship):
        ...
        assert self.graph_store.graph.has_edge(
            sample_relationship.source_entity_id,
            sample_relationship.target_entity_id
        )
    ...
    def test_get_related_entities(self, sample_entities, sample_relationship):
        ...
        related_to_python = self.graph_store.get_related_entities("tech_python", max_depth=1)
        assert "tech_flask" in related_to_python
        
        # Test backward relationship
        related_to_flask = self.graph_store.get_related_entities("tech_flask", max_depth=1)
    ...
    def test_get_related_entities_max_depth(self, sample_entities):
        ...
        entityA = Entity(id="a", name="A", entity_type=EntityType.CONCEPT, chunk_ids={"chunk1"})
        entityB = Entity(id="b", name="B", entity_type=EntityType.CONCEPT, chunk_ids={"chunk1"})
        entityC = Entity(id="c", name="C", entity_type=EntityType.CONCEPT, chunk_ids={"chunk1"})
        ...
        rel_ab = Relationship(
            ...
            relation_type=RelationType.RELATES_TO, chunk_ids={"chunk1"}
        )
        rel_bc = Relationship(
    ...
    def test_find_shortest_path(self, sample_entities, sample_relationship):
        ...
        unconnected_entity = Entity(
            ...
            entity_type=EntityType.CONCEPT, chunk_ids={"chunk3"}
        )
    ...
    def test_get_entity_centrality(self, sample_entities):
        ...
        relationships = [
            Relationship("tech_python", "tech_flask", RelationType.USES, chunk_ids={"chunk1"}),
            Relationship("course_webdev", "tech_python", RelationType.TEACHES, chunk_ids={"chunk1"}),
            Relationship("course_webdev", "tech_flask", RelationType.TEACHES, chunk_ids={"chunk1"})
        ]
    ...
    def test_serialization_round_trip(self, sample_entities, sample_relationship):
        ...
        assert new_graph.graph.number_of_edges() == self.graph_store.graph.number_of_edges()
    ...
    def test_load_invalid_json(self):
        ...
        with pytest.raises(ValueError, match="Failed to load graph data"):
            self.graph_store.load_from_json('{"invalid": "structure"}')
