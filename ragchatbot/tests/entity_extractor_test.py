...
class TestEntityExtractor:
    ...
    def test_extract_tool_entities(self):
        ...
        # Git and Docker are classified as technologies, VSCode as tool
        tech_entities = [e for e in entities if e.entity_type == EntityType.TECHNOLOGY]
    ...
    def test_extract_code_entities(self):
        ...
        # Based on current implementation, handleClick is extracted as a concept
        assert "handleClick" in concept_names

        # Also check that some code-related tools are detected
        tool_entities = [e for e in entities if e.entity_type == EntityType.TOOL]
        # The extractor found 'Cli' which relates to code examples
    ...
    def test_extract_relationships(self, sample_course_chunk):
        ...
        course_lesson_rels = [
            r for r in relationships if r.relation_type == RelationType.PART_OF
        ]
        assert len(course_lesson_rels) > 0

        # Check for teaching relationships
        teaching_rels = [
            r for r in relationships if r.relation_type == RelationType.TEACHES
        ]
    ...
    def test_entity_id_generation(self):
        ...
        id3 = self.extractor._generate_entity_id(
            "python", EntityType.TECHNOLOGY
        )  # Different case
        id4 = self.extractor._generate_entity_id(
    ...
    def test_merge_relationships(self):
        ...
        merged_rels = self.extractor.merge_relationships(
